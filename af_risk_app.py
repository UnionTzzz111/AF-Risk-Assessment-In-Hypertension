...
# 过滤掉 None 和 NaN 值，确保显示参数的清晰性
filtered_xgboost_params = {
    k: v for k, v in xgboost_classifier_model.get_params().items()
    if v is not None and not (isinstance(v, (float, np.number)) and np.isnan(v))
}
