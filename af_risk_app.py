...
st.markdown("""
This application predicts the risk of Atrial Fibrillation (AF) based on patient features using a pre-trained XGBoost model.
...
st.sidebar.markdown("---")
if st.sidebar.button("Predict AF Risk", type="primary"):
    ...
    if prediction_class == 1:
        st.error("⚠️ **Prediction:** High Risk of Atrial Fibrillation (AF)")
        st.write("Based on the input features, the model predicts a high risk of Atrial Fibrillation.")
    else:
        st.success("✅ **Prediction:** Low Risk of Atrial Fibrillation (AF)")
        st.write("Based on the input features, the model predicts a low risk of Atrial Fibrillation.")

    st.markdown("""
    ---
    **Disclaimer:** This is a predictive model for informational purposes only and should not be used as a substitute for professional medical advice, diagnosis, or treatment. Always consult with a qualified healthcare provider for any health concerns.
    """)

st.markdown("---")

# ===========================
# Main Content Area: Model Configuration Details (THIS SECTION IS REMOVED)
# ===========================
# This section has been completely removed as per user request.
# The original content to display model parameters was here.

st.markdown("---") # This markdown separator is now standalone, or you might choose to remove it too if not needed.

st.header("About This Application")
st.markdown("""
This application leverages a pre-trained XGBoost classifier to assess the risk of Atrial Fibrillation (AF).
The model was trained on a dataset of patient features and has been optimized to provide a robust prediction.

**Key Components:**
- **XGBoost Classifier:** A powerful gradient boosting algorithm known for its performance and speed.
- **StandardScaler:** Used to standardize continuous features, ensuring they contribute equally to the model.
- **Optimal Threshold:** A classification threshold determined using Youden's Index on a validation set to balance sensitivity and specificity.
