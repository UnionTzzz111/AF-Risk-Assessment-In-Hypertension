...
# ===========================
# Core Function for Data Loading, Model Training, and Evaluation (Cached)
# ===========================
@st.cache_resource(
    show_spinner="⏳ Loading data, training XGBoost model, and evaluating (this may take a few minutes)...")
def load_all_data_train_models_and_evaluate():
    st.info("🔄 **Step 1/3**: Loading data...")

    # 1. Read Data
    try:
        training_data_dataframe = pd.read_excel('train.xlsx')
        validation_data_dataframe = pd.read_excel('validation.xlsx')
        st.success("✓ `train.xlsx` and `validation.xlsx` data loaded successfully.")
    except Exception as e:
        st.error(
            f"❌ **Error**: Failed to load data files. Please ensure 'train.xlsx' and 'validation.xlsx' are in the same directory as `af_risk_app.py`. Error message: {e}")
        st.stop()

    # 2. Define Features and Labels
    # Use internal short names here to match Excel columns
    missing_training_columns = [col for col in all_features_for_models if col not in training_data_dataframe.columns]
    missing_validation_columns = [col for col in all_features_for_models if
                                  col not in validation_data_dataframe.columns]
    if missing_training_columns or missing_validation_columns:
        # Display full names for clarity in the error message
        missing_training_display = [feature_name_display_map.get(col, col) for col in missing_training_columns]
        missing_validation_display = [feature_name_display_map.get(col, col) for col in missing_validation_columns]
        st.error(
            f"❌ **Error**: Missing feature columns in training or validation sets:\nMissing in training set: {missing_training_display}\nMissing in validation set: {missing_validation_display}")
        st.stop()

    features_train_original = training_data_dataframe[all_features_for_models].copy()
    labels_train_original = training_data_dataframe[target]
    features_validation_original = validation_data_dataframe[all_features_for_models].copy()
    labels_validation_original = validation_data_dataframe[target]

    st.info("🔄 **Step 2/3**: Preprocessing all features (uniform standardization)...")

    # Check if 'Gender' column is numeric (using internal short name)
    if 'Gender' in categorical_features and not pd.api.types.is_numeric_dtype(features_train_original['Gender']):
        st.error(
            "❌ **Error**: 'Gender' column is not numeric. Please ensure the 'Gender' column in `train.xlsx` and `validation.xlsx` is encoded as 0 (Male) / 1 (Female) or similar numerical values.")
        st.stop()

    feature_scaler = StandardScaler()

    features_train_processed = pd.DataFrame(
        feature_scaler.fit_transform(features_train_original[all_features_for_models]),
        columns=all_features_for_models,
        index=features_train_original.index
    )
    features_validation_processed = pd.DataFrame(
        feature_scaler.transform(features_validation_original[all_features_for_models]),
        columns=all_features_for_models,
        index=features_validation_original.index
    )

    trained_feature_names = list(all_features_for_models)

    # Use full name for display in success message
    gender_display_name = feature_name_display_map.get('Gender',
                                                       'Gender') if 'Gender' in categorical_features else 'Categorical features'
    st.success(f"✓ All features (including {gender_display_name}) have been uniformly standardized.")

    st.info("🔄 **Step 3/3**: Defining and training the XGBoost model...")

    xgboost_params_user_provided = {
        "subsample": 0.7,
        "scale_pos_weight": 1,
        "reg_lambda": 1.0,
        "reg_alpha": 0.1,
    ...
    # Train only the XGBoost model
    xgboost_classifier_model = xgb.XGBClassifier(
    ...
    all_model_results = {}
    all_roc_curve_data = {}
    all_prediction_scores_dict = {}

    try:
        st.write(f"\n--- Training and Evaluation: **XGBoost** ---")
        xgboost_classifier_model.fit(features_train_processed, labels_train_original)
        predicted_scores = xgboost_classifier_model.predict_proba(features_validation_processed)[:, 1]
        all_prediction_scores_dict['XGBoost'] = predicted_scores

        best_threshold_youden, youden_index_value = find_youden_threshold(labels_validation_original.values,
                                                                          predicted_scores)
        performance_metrics = calculate_performance_metrics(labels_validation_original.values, predicted_scores,
                                                            best_threshold_youden)
        auc_calculation_results = bootstrap_auc(labels_validation_original.values, predicted_scores,
                                                num_bootstrap_samples=1000, model_index=0)

        brier_score = brier_score_loss(labels_validation_original.values, predicted_scores)

        all_model_results['XGBoost'] = {
            'AUC': auc_calculation_results['mean'],
            'CI_lower': auc_calculation_results['ci_lower'],
            'CI_upper': auc_calculation_results['ci_upper'],
            'Brier Score': brier_score,
            'Youden Index': youden_index_value,
            'Optimal Threshold': best_threshold_youden,
            **performance_metrics
        }

        false_positive_rate, true_positive_rate, _ = roc_curve(labels_validation_original, predicted_scores)
        all_roc_curve_data['XGBoost'] = {
            'fpr': false_positive_rate, 'tpr': true_positive_rate,
            'auc': auc_calculation_results['mean'],
            'ci_lower': auc_calculation_results['ci_lower'],
            'ci_upper': auc_calculation_results['ci_upper']
        }
        st.success(f"✓ **XGBoost** model training and evaluation completed.")

    except Exception as e:
        st.error(f"❌ **Error**: An issue occurred while processing the **XGBoost** model: {e}")
        st.code(traceback.format_exc())

    return xgboost_classifier_model, feature_scaler, trained_feature_names, training_data_dataframe, validation_data_dataframe, all_model_results, all_roc_curve_data, all_prediction_scores_dict, labels_validation_original


# ===========================
# Call Cached Function on Streamlit App Startup
# ===========================
xgboost_classifier_model, feature_scaler, trained_feature_names, training_data_dataframe, validation_data_dataframe, all_model_results, all_roc_curve_data, all_prediction_scores_dict, validation_labels_original = load_all_data_train_models_and_evaluate()

if xgboost_classifier_model is None:
    st.error(
        "Application initialization failed. Please check console for error messages and ensure data files are correct.")
    st.stop()

xgboost_optimal_threshold = all_model_results.get('XGBoost', {}).get('Optimal Threshold', 0.5)
...
for feature_short_name in continuous_features:  # Iterate using short names
    min_feature_value = training_data_dataframe[feature_short_name].min()
    max_feature_value = training_data_dataframe[feature_short_name].max()
    mean_feature_value = training_data_dataframe[feature_short_name].mean()
...
# ===========================
st.header("🔬 Interactive Risk Assessment Results")  # ⬅️ 修改点 1

if st.button("🚀 AF Risk Assessment"):  # ⬅️ 修改点 2
...
st.write("The following parameters were used to train the XGBoost model:")
